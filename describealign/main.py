...
VIDEO_EXTENSIONS = set(
    ["mp4", "mkv", "avi", "mov", "webm", "m4v", "flv", "vob"]
)
AUDIO_EXTENSIONS = set(
    ["mp3", "m4a", "opus", "wav", "aac", "flac", "ac3", "mka"]
)
PLOT_ALIGNMENT_TO_FILE = True

TIMESTEP_SIZE_SECONDS = 0.16
TIMESTEP_OVERLAP_RATIO = 0.5
AUDIO_SAMPLE_RATE = 44100
MEL_COEFFS_PER_TIMESTEP = 25
DITHER_PERIOD_STEPS = 60
MIN_CORR_FOR_TOKEN_MATCH = 0.6
GAP_START_COST = 1.0
GAP_EXTEND_COST = -0.01
GAP_EXTEND_DIAG_BONUS = -0.01
SKIP_MATCH_COST = 0.1
MAX_RATE_RATIO_DIFF_ALIGN = 0.1
PREF_CUT_AT_GAPS_FACTOR = 5
MIN_DURATION_TO_REPLACE_SECONDS = 2
MIN_START_END_SYNC_TIME_SECONDS = 2
MAX_START_END_SYNC_ERR_SECONDS = 0.2
MAX_RATE_RATIO_DIFF_BOOST = 0.003
MIN_DESC_DURATION = 0.5
MAX_GAP_IN_DESC_SEC = 1.5
JUST_NOTICEABLE_DIFF_IN_FREQ_RATIO = 0.005
CATCHUP_RATE = 5
...
def get_sorted_filenames(path, extensions, alt_extensions=set([])):
    ...
    if type(path) is list:
        ...
        if os.path.isdir(path):
            ...
            if not os.path.isfile(path):
                raise RuntimeError(
                    f"No file or directory found at input path:\n  {path}"
                )
            files = [path]
    files = [
        file
        for file in files
        if os.path.splitext(file)[1][1:] in extensions | alt_extensions
    ]
    if len(files) == 0:
        error_msg = [
            f"No files with valid extensions found at input path:\n  {path}",
    ...
    files = sorted(files)
    file_types = [
        0 if os.path.splitext(file)[1][1:] in extensions else 1
        for file in files
    ]
...
def parse_audio_from_file(media_file):
    ...
    media_arr = (
        np.frombuffer(media_stream, np.int16)
        .astype(np.float32)
        .reshape((-1, 2))
        .T
    )
...
def tokenize_audio(media_arr, rate=1):
    step_size_samples = psf.sigproc.round_half_up(
        TIMESTEP_SIZE_SECONDS * rate * AUDIO_SAMPLE_RATE
    )
    window_size_seconds = TIMESTEP_SIZE_SECONDS / TIMESTEP_OVERLAP_RATIO
    window_size_samples = psf.sigproc.round_half_up(
        window_size_seconds * AUDIO_SAMPLE_RATE
    )
    fft_size_samples = 2 ** int(np.ceil(np.log2(window_size_samples)))
    get_mfcc = lambda arr: psf.mfcc(
        np.mean(arr, axis=0),
    ...
    num_timesteps = max(
        1,
        ((media_arr.shape[1] - window_size_samples - 1) // step_size_samples)
        + 2,
    )
    media_spec = np.zeros((num_timesteps, MEL_COEFFS_PER_TIMESTEP))
    chunk_size = 1000
    for chunk_index in np.arange(0, num_timesteps, chunk_size):
        chunk_bounds_samples = (
            (chunk_index) * step_size_samples,
            (chunk_index + chunk_size - 1) * step_size_samples
            + window_size_samples,
        )
        media_spec[chunk_index : chunk_index + chunk_size] = get_mfcc(
            media_arr[:, slice(*chunk_bounds_samples)]
        )
    """
  ...
  """
    timings_samples = window_size_samples / 2.0 + step_size_samples * np.arange(
        num_timesteps
    )
...
def tokenize_audio_dither(media_arr, slow_timings):
    ...
    fast_rate = 1.0 / (
        1 + 1.0 / (DITHER_PERIOD_STEPS - 2 + (np.sqrt(5) + 1) / 2.0)
    )
    ...
    fast_spec = np.delete(
        fast_spec,
        slice(DITHER_PERIOD_STEPS // 2, None, DITHER_PERIOD_STEPS),
        axis=0,
    )
    fast_timings = np.delete(
        fast_timings, slice(DITHER_PERIOD_STEPS // 2, None, DITHER_PERIOD_STEPS)
    )
...
def normalize_spec(media_spec_raw, axes=(0, 1)):
    ...
    for axis in axes:
        ...
        media_spec = media_spec / (
            norm_func(media_spec, axis=axis, keepdims=True) + 1e-10
        )
    return media_spec
...
def rough_align(video_spec, audio_desc_spec, video_timings, audio_desc_timings):
    pred_map = {
        0: lambda node: (0, node[1] - 1, node[2] - 1),
    ...
    pred_matrix = np.zeros(
        (2, audio_desc_spec.shape[0], video_spec.shape[0]), dtype=np.uint8
    )
    pred_matrix[0, 1:, :2] = 0
    pred_matrix[1, 1:, :2] = 4
    pred_matrix[:, 0, :2] = [0, 5]
    ...
    for i in range(audio_desc_spec.shape[0]):
        i_mod = i % 3
        match_pred_corrs = np.hstack(
            [
                path_corrs_match[i_mod - 1][1:-1][:, None],
        ...
        path_corrs_match[i_mod][2:] = np.take_along_axis(
            match_pred_corrs, pred_matrix[0][i][2:, None], axis=1
        ).T
        corrs = np.roll(corrs, -1, axis=1)
        corrs[(i_mod + 1) % 3, :] = np.roll(
            np.dot(
                video_spec,
                audio_desc_spec[min(audio_desc_spec.shape[0] - 1, i + 1)],
            ),
            1,
        )
        fisher_infos = (
            2 * corrs[i_mod] - corrs[i_mod - 1] - corrs[(i_mod + 1) % 3]
        ) / min(0.2, TIMESTEP_SIZE_SECONDS)
        ...
        gap_pred_corrs = np.hstack(
            [
                path_corrs_match[i_mod][2:][:, None] - GAP_START_COST,
                path_corrs_gap[i_mod - 1][2:][:, None],
                path_corrs_gap[i_mod - 1][1:-1][:, None]
                - GAP_EXTEND_DIAG_BONUS
                - GAP_EXTEND_COST,
            ]
        )
        pred_matrix[1][i][2:] = np.argmax(gap_pred_corrs, axis=1)
        path_corrs_gap_no_col_skip = np.take_along_axis(
            gap_pred_corrs, pred_matrix[1][i][2:, None], axis=1
        ).flat
        pred_matrix[1][i][2:] += 4
        path_corrs_gap[i_mod][2:] = np.maximum.accumulate(
            path_corrs_gap_no_col_skip
            + GAP_EXTEND_COST * np.arange(video_spec.shape[0] - 2)
        ) - GAP_EXTEND_COST * np.arange(video_spec.shape[0] - 2)
        pred_matrix[1][i][2:][
            path_corrs_gap[i_mod][2:] > path_corrs_gap_no_col_skip
        ] = 7
        path_corrs_gap[i_mod][2:] -= GAP_EXTEND_COST

    # reconstruct optimal path by following predecessors backwards through the table
    end_node_layer = np.argmax(
        [path_corrs_match[i_mod, -1], path_corrs_gap[i_mod, -1]]
    )
    cur_node = (
        end_node_layer,
        audio_desc_spec.shape[0] - 1,
        video_spec.shape[0] - 1,
    )
    ...
    def get_fisher_info(node):
        i, j = node
        if (
            node[0] >= audio_desc_spec.shape[0] - 1
            or node[1] >= video_spec.shape[0] - 1
            or min(node) <= 0
        ):
            return 0
        info = (
            2 * np.dot(audio_desc_spec[i], video_spec[j])
            - np.dot(audio_desc_spec[i - 1], video_spec[j + 1])
            - np.dot(audio_desc_spec[i + 1], video_spec[j - 1])
        )
        info /= min(0.2, TIMESTEP_SIZE_SECONDS)
        return info
    ...
    def get_match_quality(node):
        ...
        token_correlation = np.dot(
            audio_desc_spec[node[0]], video_spec[node[1]]
        )
        fisher_info = min(max(0, get_fisher_info(node)), 10)
        return max(0, token_correlation - MIN_CORR_FOR_TOKEN_MATCH) * (
            fisher_info / 5
        )
    ...
    if len(quals) == 0 or max(quals) <= 0:
        raise RuntimeError(
            "Rough alignment failed, are the input files mismatched?"
        )
    path, quals = zip(
        *[(path, qual) for (path, qual) in zip(path, quals) if qual > 0]
    )
...
def smooth_align(path, quals, smoothness):
    ...
    x_stretch_factor = 10.0
    rotated_stretched_path = [
        (x_stretch_factor * x, y) for x, y in rotated_path
    ]
    ...
    slope_change_magnitudes = (
        np.abs(np.diff(y_diffs / x_diffs)) * x_stretch_factor
    )
    slope_change_locations = slope_change_magnitudes > MAX_RATE_RATIO_DIFF_ALIGN
    slope_change_locations[1:-1] *= np.abs(y[2:] - y[:-2]) > 5
    slope_change_costs = np.full(
        num_fit_points, smoothness / float(TIMESTEP_SIZE_SECONDS)
    )
    ...
    fit_err_coeffs = scipy.sparse.diags(
        [
            1.0 / x_diffs[:-1],
            -1.0 / x_diffs[:-1] - 1.0 / x_diffs[1:],
            1.0 / x_diffs[1:],
        ],
        offsets=[0, 1, 2],
        shape=(num_fit_points, num_fit_points + 2),
    ).tocsc()[:, 1:-1]
    A_eq = scipy.sparse.hstack(
        [
            fit_err_coeffs,
            -fit_err_coeffs,
    ...
    b_eq = y_diffs[1:] / x_diffs[1:] - y_diffs[:-1] / x_diffs[:-1]
    ...
    fit_err = (
        fit.x[:num_fit_points] - fit.x[num_fit_points : 2 * num_fit_points]
    )
    ...
    smooth_path = [
        (((x / x_stretch_factor) - y) / 2.0, ((x / x_stretch_factor) + y) / 2.0)
        for x, y in zip(x, y - fit_err)
    ]
    ...
    smoothing_std = MIN_START_END_SYNC_TIME_SECONDS / (
        2.0 * TIMESTEP_SIZE_SECONDS
    )
    ...
    smooth_err_path = list(
        itertools.dropwhile(lambda x: x[0] > max_sync_err, smooth_err_path)
    )[::-1]
    ...
    if is_synced_at_start:
        slope = (smooth_path[1][1] - smooth_path[0][1]) / (
            smooth_path[1][0] - smooth_path[0][0]
        )
        smooth_path.insert(0, (-10e10, -10e10 * slope))
    if is_synced_at_end:
        slope = (smooth_path[-1][1] - smooth_path[-2][1]) / (
            smooth_path[-1][0] - smooth_path[-2][0]
        )
...
def cap_synced_end_points(smooth_path, video_arr, audio_desc_arr):
    ...
    if smooth_path[-1][0] > 10e9:
        video_runtime = (video_arr.shape[1] - 2.0) / AUDIO_SAMPLE_RATE
        audio_runtime = (audio_desc_arr.shape[1] - 2.0) / AUDIO_SAMPLE_RATE
        slope = smooth_path[-1][1] / smooth_path[-1][0]
        new_end_point = (
            audio_runtime,
            smooth_path[-2][1] + (audio_runtime - smooth_path[-2][0]) * slope,
        )
        if new_end_point[1] > video_runtime:
            new_end_point = (
                smooth_path[-2][0]
                + (video_runtime - smooth_path[-2][1]) / slope,
                video_runtime,
            )
...
def plot_alignment(
    plot_filename_no_ext, path, smooth_path, quals, runs, bad_clips, ad_timings
):
    scatter_color = [0.2, 0.4, 0.8]
    lcs_rgba = np.zeros((len(quals), 4))
    lcs_rgba[:, :3] = np.array(scatter_color)[None, :]
    lcs_rgba[:, 3] = np.minimum(1, np.array(quals) * 500.0 / len(quals))
    audio_times, video_times = np.array(path).T.reshape((2, -1))
    audio_offsets = audio_times - video_times

    def expand_limits(start, end, ratio=0.01):
        average = (end + start) / 2.0
        half_diff = (end - start) / 2.0
        half_diff *= 1 + ratio
        return (average - half_diff, average + half_diff)

    plt.xlim(expand_limits(*(0, np.max(video_times) / 60.0)))
    plt.ylim(
        expand_limits(
            *(
                np.min(audio_offsets) - TIMESTEP_SIZE_SECONDS / 2.0,
                np.max(audio_offsets) + TIMESTEP_SIZE_SECONDS / 2.0,
            )
        )
    )
    plt.scatter(
        video_times / 60.0, audio_offsets, s=3, c=lcs_rgba, label="LCS Matches"
    )
    ...
    with open(plot_filename_no_ext + ".txt", "w") as file:
        ...
        for clip_start, clip_end in rough_clips:
            ...
            slope = (video_end - video_start) / (
                audio_desc_end - audio_desc_start
            )
            ...
            print(
                f"Rate change of {(slope-1.)*100:6.1f}% from {str_from_time(video_start)} to "
                + f"{str_from_time(video_end)} aligning with audio from "
                + f"{str_from_time(audio_desc_start)} to {str_from_time(audio_desc_end)}",
                file=file,
            )


# use the smooth alignment to replace runs of video sound with corresponding described audio
def replace_aligned_segments(
    video_arr, audio_desc_arr, smooth_path, runs, no_pitch_correction=False
):
    ...
    def get_interped_segment(run, interp):
        segment = []
        for clip in run:
            num_samples = int(y[clip[1]] * AUDIO_SAMPLE_RATE) - int(
                y[clip[0]] * AUDIO_SAMPLE_RATE
            )
            clip_bounds = np.array((x[clip[0]], x[clip[1]])) * AUDIO_SAMPLE_RATE
            sample_points = np.linspace(
                *clip_bounds, num=num_samples, endpoint=False
            )
    ...
    for run in runs:
        ...
        anchor_points = (
            np.array(
                (
                    np.array(x)[anchor_point_path_indices],
                    np.array(y)[anchor_point_path_indices],
                )
            )
            * AUDIO_SAMPLE_RATE
        ).astype(int)
        ...
        for clip_index, (clip, slope) in enumerate(zip(run, slopes)):
            ...
            if (
                no_pitch_correction
                or np.abs(1 - slope) <= JUST_NOTICEABLE_DIFF_IN_FREQ_RATIO
            ):
                stretched_audio = get_interped_segment(
                    [clip], audio_desc_arr_interp
                )
            else:
                anchor_point_pair = anchor_points[
                    :, clip_index : clip_index + 2
                ].copy()
                ...
                stretched_audio = pytsmod.wsola(
                    audio_desc_arr, anchor_point_pair
                )
            video_arr[
                :, slice(*anchor_points[1, clip_index : clip_index + 2])
            ] = stretched_audio
...
def detect_describer(
    ...
    slopes = (
        interp(video_timings + 1e-5) - interp(video_timings - 1e-5)
    ) / 2e-5
    median_slope = np.median(slopes)
    aligned_mask = np.abs(slopes - median_slope) < MAX_RATE_RATIO_DIFF_ALIGN
    well_aligned_mask = (
        np.abs(slopes - median_slope) < MAX_RATE_RATIO_DIFF_BOOST
    )
    ...
    speech_mask = (corrs < 0.2) * audio_desc_loud
    ...
    audio_desc_spec = np.clip(audio_desc_spec / 6.0, -1, 1)
    video_spec = normalize_spec(video_spec_raw, axes=(0,))
    video_spec = np.clip(video_spec / 6.0, -1, 1)
    ...
    def make_log_pdfs(arr):
        ...
        uniform_prior_strength_per_bin = (
            uniform_prior_strength_per_spot / float(bins_per_spot)
        )
    ...
    nondesc_log_pdfs, bin_edges = make_log_pdfs(
        diff_spec[(~speech_mask) * well_aligned_mask]
    )
    lratio_lookup = desc_log_pdfs - nondesc_log_pdfs
    lratios = lratio_lookup[
        np.fromfunction(lambda i, j: j, diff_spec.shape, dtype=int),
        np.digitize(diff_spec, bin_edges, right=True) - 1,
    ]
    ratio_desc_to_nondesc = np.sum(speech_mask * well_aligned_mask) / (
        np.sum((~speech_mask) * well_aligned_mask) + 1.0
    )
    ...
    c = np.hstack(
        [
            np.ones(num_fit_points) / pos_err_cost_factor,
            np.ones(num_fit_points) / neg_err_cost_factor,
            np.ones(num_fit_points - 1) / 2.0,
            np.ones(num_fit_points - 1) / 2.0,
        ]
    )
    fit_err_coeffs = scipy.sparse.diags(
        [-np.ones(num_fit_points), np.ones(num_fit_points)],
        offsets=[0, 1],
        shape=(num_fit_points - 1, num_fit_points),
    ).tocsc()
    A_eq = scipy.sparse.hstack(
        [
            fit_err_coeffs,
    ...
    if not fit.success:
        print(fit)
        raise RuntimeError(
            "Describer Voice Detection L1-Min Optimization Failed!"
        )
    ...
    speech_mask = smooth_desc_locations > 1.0 - 1.5 * detect_sensitivity
    ...
    boost_mask = smooth_desc_locations > 1.0 - 1.5 * boost_sensitivity
...
def encode_fit_as_ffmpeg_expr(
    smooth_path, clips, video_offset, start_key_frame
):
    ...
    if start_skip > 0:
        ...
        catchup_spread = 1.0 / CATCHUP_RATE
        setts_cmd.append(
            f"+clip(TS-STARTPTS,0,{start_skip*(1+catchup_spread)}/TB)*{-1./(1+catchup_spread)}"
        )
    elif video_offset < 0:
        # if the audio starts before the video, stretch the first frame of the video back to meet it
        setts_cmd.append(
            f"+clip(TS-STARTPTS,0,{-video_offset/10000.}/TB)*10000"
        )
    ...
    for clip_start, clip_end in clips:
        ...
        setts_cmd.append(
            f"+clip(TS-STARTPTS-{video_start:.4f}/TB,0,{max(0,video_length):.4f}/TB)*{slope-1:.9f}"
        )
    setts_cmd.append(")")
...
def get_ffmpeg():
    return static_ffmpeg.run._get_or_fetch_platform_executables_else_raise_no_lock()[
        0
    ]
...
def get_closest_key_frame_time(video_file, time):
    ...
    key_frame_times = np.array(
        [float(frame["pts_time"]) for frame in key_frames] + [0]
    )
...
def write_replaced_media_to_disk(
    output_filename,
    media_arr,
    video_file=None,
    audio_desc_file=None,
    setts_cmd=None,
    start_key_frame=None,
):
    print("Writing output file")
    if audio_desc_file is None:
        media_input = ffmpeg.input(
            "pipe:",
            format="s16le",
            acodec="pcm_s16le",
            ac=2,
        ...
        if (
            video_file is None
            or os.path.splitext(output_filename)[1][1:] in AUDIO_EXTENSIONS
        ):
            write_command = ffmpeg.output(
                media_input, output_filename, loglevel="fatal"
            ).overwrite_output()
        else:
            ...
            write_command = ffmpeg.output(
                ...
                loglevel="fatal",
                **{"c:a:0": "aac", "disposition:a:0": "default"},
            ).overwrite_output()
        ffmpeg_caller = write_command.run_async(
            pipe_stdin=True, cmd=get_ffmpeg()
        )
        ...
        audio_desc_streams = ffmpeg.probe(
            audio_desc_file,
            cmd=get_ffprobe(),
            select_streams="a",
            show_entries="format=duration",
        )["streams"]
        audio_desc_duration = max(
            [float(stream["duration"]) for stream in audio_desc_streams]
        )
        ...
        if (
            ...
            audio_codec = (
                "copy"
                if os.path.splitext(audio_desc_file)[1] != ".wav"
                else "aac"
            )
            write_command = ffmpeg.output(
                media_input,
            ...
            format = (
                "matroska"
                if os.path.splitext(output_filename)[1] == ".mkv"
                else "nut"
            )
            write_command = ffmpeg.output(
                original_video,
            ...
            ffmpeg_caller = write_command.run_async(
                pipe_stdout=True, cmd=get_ffmpeg()
            )
            pipe_input = ffmpeg.input(
                "pipe:", format=format, thread_queue_size="512"
            )
            write_command2 = ffmpeg.output(
                media_input,
                pipe_input,
                output_filename,
                c="copy",
            ...
            ffmpeg_caller2 = write_command2.run_async(
                pipe_stdin=True, cmd=get_ffmpeg()
            )
            while True:
...
def combine(
    ...
    keep_non_ad=False,
    boost=0,
    ad_detect_sensitivity=0.6,
    boost_sensitivity=0.4,
    yes=False,
    prepend="ad_",
    no_pitch_correction=False,
    output_dir=default_output_dir,
    alignment_dir=default_alignment_dir,
    extension="copy",
    display_func=None,
):
    video_files, video_file_types = get_sorted_filenames(
        video, VIDEO_EXTENSIONS, AUDIO_EXTENSIONS
    )

    if yes == False and sum(video_file_types) > 0:
        print("")
        print(
            "One or more audio files found in video input. Was this intentional?"
        )
    ...
    if len(video_files) != len(audio_desc_files):
        error_msg = [
            "Number of valid files in input paths are not the same.",
    ...
    if not is_ffmpeg_installed():
        display(
            "Downloading and installing ffmpeg (media editor, 50 MB download)...",
            display_func,
        )
        get_ffmpeg()
        if not is_ffmpeg_installed():
    ...
    for video_file, audio_desc_file, video_filetype in zip(
        video_files, audio_desc_files, video_file_types
    ):
        ...
        output_filename = (
            prepend + os.path.splitext(os.path.split(video_file)[1])[0] + ext
        )
        ...
        audio_desc_spec_raw, audio_desc_timings = tokenize_audio_dither(
            audio_desc_arr, video_timings
        )
        ...
        path, quals = rough_align(
            video_spec, audio_desc_spec, video_timings, audio_desc_timings
        )

        print("Running smooth alignment")
        smooth_path, runs, bad_clips, clips = smooth_align(
            path, quals, smoothness
        )
        ...
        if stretch_audio:
            ...
            if keep_non_ad or boost != 0:
                ...
                outputs = detect_describer(
                    video_arr,
                    video_spec,
                    video_spec_raw,
                    video_timings,
                    smooth_path,
                    ad_detect_sensitivity,
                    boost_sensitivity,
                )
            ...
            if video_filetype == 1:
                raise RuntimeError(
                    "Argument --stretch_audio is required when both inputs are audio files."
                )
            if os.path.splitext(output_filename)[1][1:] in AUDIO_EXTENSIONS:
                raise RuntimeError(
                    "Argument --stretch_audio is required when output file extension is an audio filetype."
                )
            print("Getting video offset")
            video_offset = np.diff(smooth_path[clips[0][0]])[0]
            print("Getting starting keyframe")
            start_key_frame = get_closest_key_frame_time(
                video_file, video_offset
            )
            print("Encoding fit expression")
            setts_cmd = encode_fit_as_ffmpeg_expr(
                smooth_path, clips, video_offset, start_key_frame
            )
            write_replaced_media_to_disk(
                output_filename,
        ...
        del video_arr
        if PLOT_ALIGNMENT_TO_FILE:
            plot_filename_no_ext = os.path.join(
                alignment_dir, os.path.splitext(os.path.split(video_file)[1])[0]
            )
            plot_alignment(
                plot_filename_no_ext,
                path,
                smooth_path,
                quals,
                runs,
...
def read_config_file(config_path):
    ...
    settings = {
        "smoothness": config.getfloat("alignment", "smoothness", fallback=50),
        "stretch_audio": config.getboolean(
            "alignment", "stretch_audio", fallback=False
        ),
        "keep_non_ad": config.getboolean(
            "alignment", "keep_non_ad", fallback=False
        ),
        "boost": config.getfloat("alignment", "boost", fallback=0),
        "ad_detect_sensitivity": config.getfloat(
            "alignment", "ad_detect_sensitivity", fallback=0.6
        ),
        "boost_sensitivity": config.getfloat(
            "alignment", "boost_sensitivity", fallback=0.4
        ),
        "prepend": config.get("alignment", "prepend", fallback="ad_"),
        "no_pitch_correction": config.getboolean(
            "alignment", "no_pitch_correction", fallback=False
        ),
        "output_dir": config.get(
            "alignment", "output_dir", fallback=default_output_dir
        ),
        "alignment_dir": config.get(
            "alignment", "alignment_dir", fallback=default_alignment_dir
        ),
...
def settings_gui(config_path):
    ...
    layout = [
        [sg.Text("Check tooltips (i.e. mouse-over text) for descriptions:")],
        [
            sg.Column(
                [
                    [
                        sg.Text("extension:", size=(10, 1.2), pad=(1, 5)),
                        sg.Input(
                            default_text=str(settings["extension"]),
                            size=(8, 1.2),
                            pad=(10, 5),
                            key="extension",
        ...
        [
            sg.Column(
                [
                    [
                        sg.Text("prepend:", size=(8, 1.2), pad=(1, 5)),
                        sg.Input(
                            default_text=str(settings["prepend"]),
                            size=(8, 1.2),
                            pad=(10, 5),
                            key="prepend",
        ...
        [
            sg.Column(
                [
                    [
                        sg.Text("output_dir:", size=(10, 1.2), pad=(1, 5)),
                        sg.Input(
                            default_text=str(settings["output_dir"]),
                            size=(22, 1.2),
                            pad=(10, 5),
                            key="output_dir",
                        ...
                        sg.FolderBrowse(
                            button_text="Browse Folder", key="output_browse"
                        ),
                    ]
                ]
            )
        ...
        [
            sg.Column(
                [
                    [
                        sg.Text("alignment_dir:", size=(13, 1.2), pad=(1, 5)),
                        sg.Input(
                            default_text=str(settings["alignment_dir"]),
                            size=(22, 1.2),
                            pad=(10, 5),
                            key="alignment_dir",
                        ...
                        sg.FolderBrowse(
                            button_text="Browse Folder", key="alignment_browse"
                        ),
                    ]
                ],
                pad=(2, 7),
        ...
        [
            sg.Column(
                [
                    [
                        sg.Text("smoothness:", size=(12, 1), pad=(1, 5)),
                        sg.Input(
                            default_text=str(settings["smoothness"]),
                            size=(8, 1.2),
                            pad=(10, 5),
                            key="smoothness",
        ...
        [
            sg.Checkbox(
                "stretch_audio",
                default=settings["stretch_audio"],
                key="stretch_audio",
                change_submits=True,
        ...
        [
            sg.Column(
                [
                    [
                        sg.Text("boost:", size=(6, 1), pad=(1, 5)),
                        sg.Input(
                            default_text=str(settings["boost"]),
                            size=(8, 1.2),
                            pad=(10, 5),
                            key="boost",
                            disabled=not settings["stretch_audio"],
        ...
        [
            sg.Column(
                [
                    [
                        sg.Text(
                            "ad_detect_sensitivity:", size=(21, 1.2), pad=(2, 5)
                        ),
                        sg.Input(
                            default_text=str(settings["ad_detect_sensitivity"]),
                            size=(8, 1.2),
                            pad=(10, 5),
                            key="ad_detect_sensitivity",
                            disabled=not settings["stretch_audio"],
        ...
        [
            sg.Column(
                [
                    [
                        sg.Text(
                            "boost_sensitivity:", size=(17, 1.2), pad=(1, 5)
                        ),
                        sg.Input(
                            default_text=str(settings["boost_sensitivity"]),
                            size=(8, 1.2),
                            pad=(10, 5),
                            key="boost_sensitivity",
                            disabled=not settings["stretch_audio"],
        ...
        [
            sg.Checkbox(
                "no_pitch_correction",
                default=settings["no_pitch_correction"],
                key="no_pitch_correction",
                disabled=not settings["stretch_audio"],
        ...
        [
            sg.Column(
                [[sg.Submit("Save", pad=(40, 3)), sg.Button("Cancel")]],
                pad=((135, 3), 10),
            )
        ],
    ]
    settings_window = sg.Window(
        "Settings - describealign", layout, font=("Arial", 16), finalize=True
    )
    settings_window["extension"].set_focus()
    while True:
        event, values = settings_window.read()
        if (
            event in (sg.WIN_CLOSED, "Cancel")
            or settings_window.TKrootDestroyed
        ):
            break
        if event == "stretch_audio":
            # work around bug in PySimpleGUIWx's InputText Update function where enabling/disabling are flipped
            if IS_RUNNING_WINDOWS:
                settings_window["boost"].Update(
                    disabled=values["stretch_audio"]
                )
                settings_window["ad_detect_sensitivity"].Update(
                    disabled=values["stretch_audio"]
                )
                settings_window["boost_sensitivity"].Update(
                    disabled=values["stretch_audio"]
                )
            else:
                settings_window["boost"].Update(
                    disabled=not values["stretch_audio"]
                )
                settings_window["ad_detect_sensitivity"].Update(
                    disabled=not values["stretch_audio"]
                )
            ...
            settings_window["keep_non_ad"].Update(
                disabled=not values["stretch_audio"]
            )
            settings_window["no_pitch_correction"].Update(
                disabled=not values["stretch_audio"]
            )
        if event == "Save":
...
def combine_gui(video_files, audio_files, config_path):
    ...
    layout = [[output_textbox], [sg.Button("Close", pad=(360, 5))]]
    combine_window = sg.Window(
        "Combining - describealign",
        layout,
        font=("Arial", 16),
        disable_close=True,
        finalize=True,
    ...
    proc = multiprocessing.Process(
        target=combine_print_exceptions,
    ...
    while True:
        ...
        if event == "Close":
            if not proc.is_alive():
                combine_window.DisableClose = False
                break
            selection = sg.PopupYesNo(
                "Combiner is still running, stop it and close anyway?"
            )
            if selection != "Yes":
                continue
            proc.terminate()
...
def main_gui():
    ...
    all_audio_file_types = [
        ("All Audio File Types", "*." + ";*.".join(AUDIO_EXTENSIONS)),
    ]
    all_video_file_types = [
        ("All Video File Types", "*." + ";*.".join(VIDEO_EXTENSIONS)),
    ]
    all_video_and_audio_file_types = [
        (
    ...
    video_and_audio_file_types = [
        (ext, "*." + ext) for ext in VIDEO_EXTENSIONS
    ] + audio_file_types
    ...
    if IS_RUNNING_WINDOWS:
        file_fix = lambda file_types: file_types[:1] + [
            ("|" + type[0], type[1]) for type in file_types[1:]
        ]
    ...
    layout = [
        ...
        [
            sg.Column(
                [
                    [
                        sg.Text("Video Input:", size=(11, 2), pad=(1, 5)),
                        sg.Input(
                            size=(35, 1.2),
                            pad=(10, 5),
                            key="-VIDEO_FILES-",
                        ...
                        sg.FilesBrowse(
                            button_text="Browse Video",
                            file_types=video_and_audio_file_types,
                            tooltip="Select one or more video files",
                        ),
                    ]
                ],
                pad=(2, 7),
        ...
        [
            sg.Column(
                [
                    [
                        sg.Text("Audio Input:", size=(11, 2), pad=(1, 5)),
                        sg.Input(
                            size=(35, 1.2),
                            pad=(10, 5),
                            key="-AUDIO_FILES-",
                        ...
                        sg.FilesBrowse(
                            button_text="Browse Audio",
                            file_types=audio_file_types,
                            tooltip="Select one or more audio files",
                        ),
                    ]
                ],
                pad=(2, 7),
                ...
                [
                    [
                        sg.Submit(
                            "Combine",
                            pad=(40, 3),
                            tooltip="Combine selected video and audio files",
                        ),
                        sg.Button(
                            "Settings",
                            tooltip="Edit settings for the GUI and algorithm.",
                        ),
                    ]
                ],
                pad=((135, 3), 10),
    ...
    while True:
        ...
        if event == "Combine":
            if (
                len(values["-VIDEO_FILES-"]) == 0
                or len(values["-AUDIO_FILES-"]) == 0
            ):
                window.disable()
...
def command_line_interface():
    ...
    parser.add_argument(
        "video", help="A video file or directory containing video files."
    )
    parser.add_argument(
        "audio", help="An audio file or directory containing audio files."
    )
    parser.add_argument(
        "--smoothness",
        type=float,
    ...
    parser.add_argument(
        "--prepend",
        default="ad_",
        help='Output file name prepend text. Default is "ad_"',
    )
    parser.add_argument(
        "--no_pitch_correction",
        action="store_true",
    ...
    combine(
        ...
        args.keep_non_ad,
        args.boost,
        args.ad_detect_sensitivity,
        args.boost_sensitivity,
        args.yes,
        args.prepend,
